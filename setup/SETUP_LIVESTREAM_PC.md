...
넣은 뒤 소스 목록에서 **카메라 위**로 올리고, 우클릭 → **변환 → 변환 초기화**(`Ctrl+R`).
캔버스와 크기가 같아 그것만으로 자리가 맞는다. **드래그로 늘리지 않는다** — 늘리면 뭉개진다.
...
| 파라미터 | 뜻 |
|---|---|
| `?pos=tl` | 좌상단(기본) · `tr` 우상단 · `bl` 좌하단 · `br` 우하단 |
| `?scale=1.25` | 배율 (기본 1) |
| `?margin=48` | 화면 가장자리 여백 px (기본 48) |
| `?demo=1` | **샘플 점수로 표시** — 경기가 없을 때 위치를 잡을 때 쓴다 |

예: `.../obs.html?pos=br&scale=1.2` (우하단, 20% 크게)

> 경기가 없으면 오버레이는 **아무것도 안 보인다**(정상). 자리를 잡으려면 `?demo=1`을 붙여
> 샘플을 띄우고 위치를 정한 뒤, 파라미터를 빼서 저장한다.
