...
#
# ⚠️ 이 파일은 반드시 'UTF-8 with BOM'으로 저장해야 한다. Windows PowerShell 5.1은
#    BOM이 없으면 .ps1을 cp949로 읽어서, 깨진 한글 바이트가 따옴표를 삼켜 파싱 자체가
#    깨진다("식 또는 문에서 예기치 않은 토큰"). 다른 스크립트들은 메시지가 영어라
#    이 함정에 안 걸릴 뿐이다. 에디터에서 인코딩을 바꾸지 말 것.
#
# 증상
